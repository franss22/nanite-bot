...
async def register(ctx: Ctx, nombre: str, piso: int):
    ...
    try:
        ...
        await ctx.send(
            f"Ya tienes el PJ {pc_name} asignado. Si quieres registrar un nuevo PJ, elimina el anterior primero."
        )
        return
    except DBNone:
...
async def reset_dt(ctx: Ctx, dt: int):
    ...
    for user in users:
        user_id = user.decode("utf-8")
        pc_name = db.get_pc_name(user_id)
        db.update_int_val(user_id, pc_name, "dt", dt)
        i += 1

    await ctx.send(f"Sumado {dt} días al dt de {i} PJs.")
...
async def backup(ctx: Ctx):
    ...
    await ctx.send(
        "Backup completado",
        file=nextcord.File(fp=io.BytesIO(txt.encode("utf-8")), filename="backup.txt"),
    )
...
async def piso(ctx: Ctx, new_floor: int):
    user_id = ctx.author.id
    pc_name = db.get_pc_name(user_id)
    success, old, updated = db.update_int_val(
        user_id, pc_name, "credits", new_floor, replace=True, can_go_negative=True
    )
    if success:
    ...
    async def send_initial_message(self, ctx, channel):
        if self.not_enough_classes:
            return await channel.send(
                f"No puedes eliminar tu única clase. Añade otra clase primero.",
                view=self,
            )
        return await channel.send(
            f'Selecciona la clase que quieres {"añadir" if self.add else "quitar"}.',
            view=self,
        )
...
async def unregister(ctx: Ctx, secure: str = ""):
    if secure == "seguro_que_quiero_borrar_mi_pj":
        ...
        await ctx.send(
            "Manda este comando con el parametro 'seguro_que_quiero_borrar_mi_pj' para desregistrar a tu PJ."
        )
